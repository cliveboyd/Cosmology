# HHT_BAO_Full_Analysis

## Source

- Local DOCX: `C:\Users\clive\Documents\Cosmology\HHT_BAO_Full_Analysis.docx`
...
Extended HHT–BAO Locking Analysis

Background

This section integrates Hilbert–Huang Transform (HHT) decomposition of Pantheon+SH0ES residuals with BAO scale tests, comparing IMF instantaneous frequencies (ω_inst) against effective BAO scales (DM/rd and DH/rd). The objective is to identify whether intrinsic residual oscillations carry signatures of BAO scale locking and to assess how Noether symmetry constraints and Λ(z) fluctuations modify these couplings.

Key Acronyms and Parameters

• HHT – Hilbert–Huang Transform• IMF – Intrinsic Mode Function• BAO – Baryon Acoustic Oscillations• DM – Transverse BAO distance• DH – Radial BAO distance• rd – Sound horizon at drag epoch• ω_inst – Instantaneous frequency from Hilbert transform• Λ(z) – Redshift-dependent dark energy fluctuation term• Noether conserve_time – Symmetry constraint freezing accretion/energy non-conservation

Method

1. Extract ω̄ (median instantaneous frequency) for IMF 2 and IMF 3 from each run's hht_sn_detail.npz.2. Regress ω̄_IMF vs 1/k_eff(z) from BAO DM/rd and DH/rd.3. Compare baseline (FRPBH), Noether-constrained (conserve_time), and Λ(z)-coupled cases.4. Evaluate Pearson correlation and p-values to test robustness.

Results

IMF 2

• Baseline FRPBH:   – DM/rd: r≈+0.10, p≈0.008 → positive correlation (transverse lock).   – DH/rd: r≈–0.10, p≈0.006 → negative correlation (orthogonal radial component).   → IMF 2 robustly tracks the BAO scale, consistent with transverse DM locking.• Noether (conserve_time):   – Correlations suppressed (p>0.1).   → Locking weakened by symmetry constraint.• Λ(z)-coupled:   – IMF 2 correlations vanish (p>0.4).   → BAO lock shifts away from IMF 2 under fluctuations.

IMF 3

• Baseline & Noether: no significant relation to BAO (p>0.4).• Λ(z)-coupled:   – Emerges as a weak candidate with r≈+0.02 vs DM/rd, though not significant.   – Suggests IMF 3 may act as a shifted harmonic mode introduced by Λ(z) fluctuations.

Heatmap Analysis

Heatmaps across IMF indices confirm IMF 2 consistently carries the BAO signal (p<1e-3 across smoothings 5–11). Λ(z) coupling introduces additional IMF 3 alignment. Noether constraints reduce locking overall.

Implications for BAO and Flat Geometry

The fact that IMF 2 locks preferentially to DM/rd (transverse) rather than DH/rd (radial) suggests that BAO as typically applied in cosmological fits may be anisotropically biased. This could allow reinterpretation of BAO calibration under a flat space-time structure, rather than enforcing ΛCDM spherical expansion symmetry. IMF decomposition highlights residual anisotropy that standard isotropic BAO averages would wash out.In this context:• Baseline IMF 2 = BAO transverse lock → consistent across smoothings.• Noether = suppresses anisotropy → symmetry damping.• Λ(z) coupling = shifts BAO correlation to IMF 3 → fluctuation-induced harmonic.

Conclusions

1. IMF 2 carries a robust BAO scale lock (p<1e-3), primarily with DM/rd.2. Noether symmetry suppresses this signal, confirming the lock depends on accretion and energy non-conservation.3. Λ(z) coupling shifts locking to IMF 3, consistent with fluctuation-driven harmonics.4. The analysis supports potential reinterpretation of BAO calibration in terms of a flat-space framework, where anisotropy in residuals reveals structure beyond standard ΛCDM isotropy assumptions.
