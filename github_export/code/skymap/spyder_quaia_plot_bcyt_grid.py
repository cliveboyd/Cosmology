...
def plot_amp(series):
    ...
    for bcut, style in zip(BCUT_LIST, styles):
        s = series[bcut]
        if s["z"].size == 0:
            continue
        plt.plot(s["z"], s["amp"], style, label=f"|b|>{int(bcut)}°")
...
def plot_amp_par(series):
    ...
    for bcut, style in zip(BCUT_LIST, styles):
        ...
        if s["z"].size == 0:
            continue
        plt.plot(s["z"], s["amp_par"], style, label=f"|b|>{int(bcut)}°")
...
def plot_frac_par(series):
    ...
    for bcut, style in zip(BCUT_LIST, styles):
        ...
        if s["z"].size == 0:
            continue
        plt.plot(s["z"], s["frac_par"], style, label=f"|b|>{int(bcut)}°")
