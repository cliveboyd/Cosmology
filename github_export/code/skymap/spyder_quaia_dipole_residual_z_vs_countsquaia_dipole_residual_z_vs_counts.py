...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 22 21:06:42 2025
...
"""
quaia_dipole_residual_z_vs_counts.py  (0V1)
...
from quaia_dipole_fit import fit_dipole_linear, dipole_summary_from_b, L_CMB_DEG, B_CMB_DEG
...
def main():
    ...
    theta, phi = hp.pix2ang(NSIDE, pix)  # theta colatitude, phi longitude
    # Convert to Galactic-like unit vectors
    x = np.sin(theta) * np.cos(phi)
    y = np.sin(theta) * np.sin(phi)
    z = np.cos(theta)
    n_hat = np.vstack([x, y, z])  # shape (3, N_good)

    # --- 3) Fit dipole to original zmean (for comparison)
    z_zero = z_good - z_good.mean()
    a0_z, b_z, _ = fit_dipole_linear(z_zero, n_hat, w=N_good)
    amp_z, l_z, b_z_deg = dipole_summary_from_b(b_z)

    # --- 4) Fit dipole to residual z
